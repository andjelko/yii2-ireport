...
class IReportRender {
    ...
    private function renderPageFooter() {
        if (isset($this->RParser->pageFooter)) {
            foreach ($this->RParser->pageFooter as $out) {
                switch ($out["hidden_type"]) {
                    case "field":
                        $this->display($out, $this->RParser->pageSetting["pageHeight"] - $this->RParser->pageFooter[0]["height"] - $this->RParser->pageSetting["bottomMargin"], true);
                        #$this->display($out, 500, true);
                        break;
                    default:
                        $this->display($out, $this->RParser->pageSetting["pageHeight"] - $this->RParser->pageFooter[0]["height"] - $this->RParser->pageSetting["bottomMargin"], false);
                        #$this->display($out, 500, false);
                        break;
                }
            }
        } else {
    ...
    private function renderLastPageFooter() {
        if (isset($this->RParser->lastPageFooter)) {
            foreach ($this->RParser->lastPageFooter as $out) {
                switch ($out["hidden_type"]) {
                    case "field":
                        $this->display($out, $this->RParser->pageSetting["pageHeight"] - $this->RParser->lastPageFooter[0]["height"] - $this->RParser->pageSetting["bottomMargin"], true);
                        break;
                    default:
                        $this->display($out, $this->RParser->pageSetting["pageHeight"] - $this->RParser->lastPageFooter[0]["height"] - $this->RParser->pageSetting["bottomMargin"], false);
                        break;
                }
            }
        }
    ...
    private function renderDetail() {
        ...
        foreach ($this->data as $row) {
            ...
            foreach ($this->RParser->detail as $compare) { //this loop is to count possible biggest Y of the coming row
                switch ($compare["hidden_type"]) {
                    case "field":
                        ...
                        if (isset($this->group[${$this->group_name}]["groupFooter"]) && (($checkpoint + ($compare["height"] * $txt)) > ($this->RParser->pageSetting["pageHeight"] - $this->RParser->groupfootheight - $this->RParser->pageSetting["bottomMargin"]))) {//check group footer existed or not
                            $this->renderPageFooter();
                            $checkpoint = $this->RParser->detail[0]["y_axis"];
                            $biggestY = 0;
                            $tempY = $this->RParser->detail[0]["y_axis"];
            ...
            if ($checkpoint + $this->RParser->detail[0]["height"] > ($this->RParser->pageSetting["pageHeight"] - $this->pageFooter[0]["height"] - $this->RParser->pageSetting["bottomMargin"])) { //check the upcoming band is greater than footer position or not
                $this->background();
                $headerY = $this->RParser->pageSetting["topMargin"] + $this->RParser->pageHeader[0]["height"];
                $this->renderPageHeader($headerY);
                $checkpoint = $this->RParser->detail[0]["y_axis"];
            ...
            if (isset($this->RParser->group) && ($this->global_pointer > 0) &&
            ...
            foreach ($this->data as $row) {
                ...
                foreach ($this->RParser->detail as $compare) { //this loop is to count possible biggest Y of the coming row
                    switch ($compare["hidden_type"]) {
                        case "field":
                            $txt = $this->analyse_expression($row["$compare[txt]"]);
                            if (isset($this->group[${$this->group_name}]["groupFooter"]) && (($checkpoint + ($compare["height"] * $txt)) > ($this->RParser->pageSetting[pageHeight] - $this->group["$this->group_name"][groupFooter][0]["height"] - $this->RParser->pageSetting["bottomMargin"]))) {
    ...
    private function renderGroupHeader($y) {
        $bandheight = $this->RParser->grouphead[0]['height'];
    ...
    private function display($arraydata, $y_axis=0, $fielddata=false) {
        ...
        elseif ($arraydata["type"] == "subreport") {
            ...
            if ($fielddata == false) {
                ...
                $this->checkoverflow(
                        $arraydata, $this->updatePageNo($this->analyse_expression($arraydata["txt"], $arraydata["isPrintRepeatedValues"])));
            }
    ...
    private function updatePageNo($s) {
        return str_replace('$this->TotPageNo()', $this->pdf->getAliasNbPages(),str_replace('$this->PageNo()', /*$this->pdf->PageNo() */$this->pdf->getAliasNumPage(), $s));
    }
    ...
    private function checkoverflow($arraydata, $txt="") {
        ...
        if ($this->print_expression_result == true) {
            ...
            if ($arraydata["writeHTML"] == 1 && $this->pdflib == "TCPDF") {
                ...
                $this->pdf->Cell(
                        $arraydata["width"], $arraydata["height"], IReportHelper::formatText($txt, $arraydata["pattern"]), $arraydata["border"], "", $arraydata["align"], $arraydata["fill"], $arraydata["link"]);
                if ($this->debuggroup == true)
                    $this->pdf->MultiCell(100, 10, "SampleText");
            }
            ...
            elseif ($arraydata["poverflow"] == "false" && $arraydata["soverflow"] == "true") {
                $this->pdf->MultiCell(
                        $arraydata["width"], $arraydata["height"], IReportHelper::formatText($txt, $arraydata["pattern"]), $arraydata["border"], $arraydata["align"], $arraydata["fill"]);
                if ($this->debuggroup == true)
                    $this->pdf->MultiCell(100, 10, "SampleText");
            }
            else {
